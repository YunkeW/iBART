...
ADD <- function(dat) {
  if (is.null(dat$unit)) {
    p <- ncol(dat$X)
    df_out <- list(X = matrix(0, nrow = nrow(dat$X), ncol = choose(p, 2) + p),
                   unit = NULL,
                   name = c())
    count <- 1

    # First do all pairwise combinations of features
    for (i in 1:(p-1)) {
      for (j in (i+1):p) {
    ...
    # Then add CONST to each feature
    const_col <- which(dat$name == "CONST")
    if (length(const_col) > 0) {
      for (i in 1:p) {
        if (i != const_col) {  # Skip if this is the CONST column itself
          df_out$X[, count] <- dat$X[, i] + dat$X[, const_col]
          df_out$name[count] <- paste0("(", dat$name[i], "+CONST)")
          count <- count + 1
        }
      }
    } else {
      # If no CONST column exists, add 1 and label it as CONST
      for (i in 1:p) {
        df_out$X[, count] <- dat$X[, i] + 1
        df_out$name[count] <- paste0("(", dat$name[i], "+CONST)")
        count <- count + 1
      }
    }
      ...
      if ((p_const > 0) & (p_tmp > 0)) {
        for (j in 1:p_tmp) {
          for (k in 1:p_const){
            ...
            df_out$name[count] <- paste0("(", name_tmp[j], "+CONST)")
            count <- count + 1
          }
        }
    ...
    # Only add "+1" if no CONST column was found in any unit group
    if (p_const == 0) {
      for (i in 1:p) {
        df_out$X[, count] <- dat$X[, i] + 1
        df_out$name[count] <- paste0("(", dat$name[i], "+CONST)")
...
ABS <- function(dat) {
  dat$X <- abs(dat$X)
  dat$name <- unname(sapply(dat$name, function(x) {
    if (grepl("\\+CONST", x)) {
      paste0("abs(", x, ")")
    } else {
      paste0("abs(", x, ")")
...
SQRT <- function(dat, apply_pos_opt_on_neg_x) {
  ...
  if (apply_pos_opt_on_neg_x) {
    ...
    dat$name <- unname(sapply(dat$name[!neg_col], function(x) {
      if (grepl("\\+CONST", x)) {
        paste0("sqrt(", x, ")")
      } else {
        paste0("sqrt(", x, ")")
